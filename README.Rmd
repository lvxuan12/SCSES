...
The other input data required for SCSES for the test data can be downloaded from ---, including genome FASTA file, annotation GTF and GFF3 file, RBP file, phast conservation file, and configure file.
...
The `cell_line.json` file downloaded previously is an example configuration file for test data. Users can modify this file or use `createConfigshiny` function to create a new configuration file.
...
A list of final imputation of PSI values will be saved to `work_path/imputation/Imputed_combined*`.
...
calcu_umap<-function(data,n_neighbors){
  ...
  v=apply(data,1,var)
  data = data[order(v,decreasing = T)[1:ceiling(nrow(data)*1)],]
...
ggplot(data = data_umap,aes(x =V1 ,y =V2))+
